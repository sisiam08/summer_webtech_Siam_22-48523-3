...
function createMultiShopOrder($customerId, $customerName, $customerEmail, $customerPhone, $deliveryAddress, $paymentMethod) {
    ...
    try {
        ...
        error_log("Creating order for user_id: $customerId, total: $grandTotal");
        
        // Create main order - using actual table structure
        ...
        error_log("Order Insert SQL: $sql");
        ...
        $check_shop_orders = $conn->query("SHOW TABLES LIKE 'shop_orders'");
        $use_shop_orders = ($check_shop_orders && $check_shop_orders->rowCount() > 0);
        ...
        foreach ($shopCartItems as $shopId => $shopCart) {
            ...
            if ($use_shop_orders) {
                ...
                $sql = "INSERT INTO shop_orders (order_id, shop_id, subtotal, delivery_charge) VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $result = $stmt->execute([$orderId, $shopId, $subtotal, $deliveryCharge]);
                
                if (!$result) {
                    throw new Exception("Failed to create shop order");
                }
                
                $shopOrderId = $conn->lastInsertId();
                error_log("Shop order created with ID: $shopOrderId for shop: $shopId");
            }
            ...
            foreach ($shopCart['items'] as $item) {
                ...
                if ($use_shop_orders && $shopOrderId) {
                    $sql = "INSERT INTO order_items (order_id, shop_order_id, product_id, quantity, price) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                    $result = $stmt->execute([$orderId, $shopOrderId, $productId, $quantity, $price]);
                } else {
                ...
                error_log("Order item added: Product $productId, Quantity $quantity, Price $price");
            }
        }
        ...
        // Clear cart - handle both database and session cart
        if (isset($_SESSION['user_id'])) {
            try {
