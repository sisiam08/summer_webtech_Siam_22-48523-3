...
try {
    ...
    if ($existingShop) {
        ...
        $updateStmt = $pdo->prepare("
            ...
            WHERE owner_id = ?
        ");
        $result = $updateStmt->execute([$name, $description, $address, $phone, $email, $userId]);
    } else {
        // Insert new shop record
        $insertStmt = $pdo->prepare("
            INSERT INTO shops (owner_id, name, description, address, phone, email, created_at)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $result = $insertStmt->execute([$userId, $name, $description, $address, $phone, $email]);
    }
